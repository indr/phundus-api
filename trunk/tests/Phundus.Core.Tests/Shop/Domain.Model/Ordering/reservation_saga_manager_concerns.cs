namespace Phundus.Core.Tests.Shop.Domain.Model.Ordering
{
    ...
    public class reservation_saga_manager_concerns : saga_manager_concern<ReservationSaga, ReservationSagaManager>
    {
        ...
        protected static Guid _orderItemId2 = Guid.NewGuid();
        ...
        protected static ReservationSaga _saga2;

        private Establish ctx = () =>
        {
            _correlationId = _orderItemId;
            _saga2 = mock.stub<ReservationSaga>();
            _sagaRepository.WhenToldTo(x => x.GetById<ReservationSaga>(_orderItemId2)).Return(_saga2);
        };

        private Because of = () =>
        {
            sut.Handle((dynamic) _event);
        };
    }
    ...
    {
        private Establish ctx = () =>
            _event = new OrderClosed(_initiatorId, _organizationId, _orderId, new List<Guid> {_orderItemId, _orderItemId2 });

        public It should_get_saga_by_reservation_id = () => AssertRepositoryGetById(_orderItemId);

        public It should_get_saga_2_by_reservation_id = () => AssertRepositoryGetById(_orderItemId2);

        public It should_tell_saga_to_transition = () => AssertTransition();

        public It should_tell_saga_2_to_transition = () => AssertTransition(_saga2);
    }

    public class when_order_rejected_is_handled : reservation_saga_manager_concerns
    {
        private Establish ctx = () =>
            _event = new OrderRejected(_initiatorId, _organizationId, _orderId, new List<Guid> { _orderItemId, _orderItemId2 });
